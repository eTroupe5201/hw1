...
const Product = class Product {
    ...
    constructor(item){
        ...
        this.name = parsed[0];
        this.price =  parseFloat(parsed[1]);
    ...
    static inStock = (products) => {
       ...
       for(var i in products){
        if(products[i].availability == 'In Stock'){
            currentlyInStock.push(products[i]);
        }}
    ...
    static halfOff = (products) => {
        ...
        let clonedArr = JSON.parse(JSON.stringify(products));
        for(var i in products){
            clonedArr[i].price = clonedArr[i].price / 2;
        }
        return clonedArr;
        };
    ...
    static printProducts = (products) => {
         ...
         for(var i in products){
            if(products[i].availability === 'In Stock'){
                console.log(`Product: ${products[i].name}, Cost: ${new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD', currencyDisplay: 'narrowSymbol'}).format(products[i].price)}, Availability: Yes`);
            }else{
                console.log(`Product: ${products[i].name}, Cost: ${new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD', currencyDisplay: 'narrowSymbol'}).format(products[i].price)}, Availability: No`);
            } }}};
